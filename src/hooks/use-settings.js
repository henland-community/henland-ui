...
async function fetchSettings() {
  const [
    ...
    //logosResponse,
    //logosPrideResponse,
    henlandRestrictedListResponse,
    henlandPermittedListResponse,
    nsfwResponse,
    ...
    axios.get(report_url('restricted.json')), // Henland list of restricted accounts
    axios.get(report_url('permitted.json')), // Henland list of accounts that override HEN's restricted list
    axios.get(report_url('nsfw.json')), // Henland list of NSFW tokens that are added by the moderation team
    axios.get(report_url('photosensitive.json')), // Henland list of Photosensitive tokens that are added by the moderation team
    axios.get(report_url('under_review.json')), // Henland list of under review accounts added by the moderation team
    axios.get(report_url('ignore_uri.json')), // Henland list of uri to ignore added by the moderation team
    axios.get(report_url('fund_feed_ignored_addresses.json')), // Henland list of wallets to ignore only from feeds (created to avoid fundraiser tag abusers)
  ])
  ...
  const walletBlockMap = filterWalletBlockList(
    [henlandRestrictedListResponse.data],
    [henlandPermittedListResponse.data]
  )
  return {
